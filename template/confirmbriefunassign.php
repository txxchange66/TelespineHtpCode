...
function unassignbriefintake(provider_id,patient_id)
{
    $.post('../index.php?action=unassignbriefIntake',{patient_id:patient_id,provider_id:provider_id}, function(data,status){
                    if( status == "success" ){
                     ...
                     parent.document.getElementById("unassign_brifeintake_but").style.display= 'none';  
                     parent.document.getElementById("brifeintake_but").style.display='block';  
                     parent.document.getElementById("brifeintake_but").style.fontSize = '11px';
                     parent.document.getElementById("brifeintake_but").style.cssFloat = 'right'
                      $("#intakeblbl").html('Assign brief Intake Paperwork');
                     if(parent.document.getElementById("intake_but").value== 'Assign'){
                	   parent.document.getElementById("intake_but").disabled= false;   
                    } 
...
<div id="popup_body" style="margin:0px; padding:0px; vertical-align:top;">
  <table width="99%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <span>Do you want to unassign brief intake paperwork?</span>   
      </td>
    </tr>
    <tr>
        <td align="center">&nbsp;</td>
    ...
    <tr>
        <td colspan="2"><div align="center"><input type="button" value="  Ok  " onclick="unassignbriefintake(<?php echo $_REQUEST['provider_id'] ?>,<?php echo $_REQUEST['patient_id'] ?>);"/>&nbsp;&nbsp;<input type="button" value="  Cancel  " onclick="parent.parent.GB_hide();"/></div></td>
    </tr>
  </table>
</div>
