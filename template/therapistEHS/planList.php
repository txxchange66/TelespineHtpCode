...
<script language="JavaScript">
	...
	//alert(a);
		...
		if(a == 'plan_customize' ){
			action = 'createNewEhsPlan';
			act = 'plan_edit';
			if (c) window.location.href = 'index.php?action=' + action + '&act=' + act + '&plan_id=' + plan_id + '&type=finish';
		}
		if(a == 'deletePlan' ){
			action = 'therapistEhsPlan';
			act = 'deletePlan';
			if (c) window.location.href = 'index.php?action=' + action + '&act=' + act + '&plan_id=' + plan_id;
		}
	...
	<td style="valign:middle;">
		...
		<form method="POST" action="index.php?action=therapistEhsPlan&path=<!path>" name="filter" onSubmit="return search_key(document.filter.search.value);" onMouseOver="help_text(this, 'Please enter text to restrict the list to only items that contain the text entered')">
			<label for="search">Search:</label>
			<input type="text" size="20" maxlength="250" name="search" value="">
			<input type="submit" size="20" name="sub" value=" Search ">
			<input type="button" size="20" name="clear" value="   Clear   " onclick="location.href='index.php?action=therapistEhsPlan&path=<!path>'">
