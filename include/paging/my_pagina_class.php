...
paging class to implement paging using mysql database



*************************************************************************/

require_once("include/paging/db_config.php");

//error_reporting(E_ALL); // only for testing

require_once("include/db.class.php");

class MyPagina extends  db {
	...
	function MyPagina($rows = 0,$action='',$div_name='',$search='',$skipValue=array(),$rows_on_page='') {
	...
	function get_total_rows() {
        ...
        if($this->sqlcount!='')
        {
        $tmp_result = @mysql_query($this->sqlcount);  
        $all_rows = @mysql_result($tmp_result,0,0); 
        }else{
	...
	function connect_db() {
		...
		$conn_str = @mysql_connect(db::$config['dbconfig']['db_host_name'], db::$config['dbconfig']['db_user_name'], db::$config['dbconfig']['db_password']);

		@mysql_select_db(db::$config['dbconfig']['db_name'], $conn_str);
		...
		if (!empty($_SERVER['QUERY_STRING'])) {
			...
			foreach ($parts as $val) {
				...
				if (stristr($val, $curr_var) == false)  {
					$arr = explode("=",$val);
					if(is_array($this->skipValue) && in_array($arr[0],$this->skipValue)){
	...
	function navigation($separator = "| ", $css_current = "", $back_forward = false, $use_images = false) {
		...
		if ($curr_pages <= $all_pages && $curr_pages >= 0) {
			...
			if($all_pages >= 1) {
				...
				$navi_string = ($curr_pages > 0) ?  "<a href='index.php?action=".$this->action."&search=".urlencode($this->search)."&".$var."=".$backward.$this->rebuild_qs($var)."'>". $lbl_backward."</a>&nbsp;" :  $lbl_backward."&nbsp";
				...
				if (!$back_forward) {
					...
					for($a = $start + 1; $a <= $end; $a++){
						...
						if ($theNext != $curr_pages) {
							...
							$navi_string .= "<a href='index.php?action=".$this->action."&search=".urlencode($this->search)."&".$var."=".$theNext.$this->rebuild_qs($var)."'>";
				...
				$navi_string .= ($curr_pages < $all_pages) ? "&nbsp;<a href='index.php?action=".$this->action."&search=".urlencode($this->search)."&".$var."=".$forward.$this->rebuild_qs($var)."'>".$lbl_forward."</a>" : "&nbsp;".$lbl_forward;
