...
*************************************************************************/
require($_SERVER['DOCUMENT_ROOT']."/klass/include/paging/db_config.php");
...
class MyPagina {
	...
	// constructor
	function MyPagina($rows = 0,$action='',$div_name='',$where='') {
		$this->connect_db();
		$this->max_rows = $rows;
		$this->get_var = QS_VAR;
	...
	function navigation($separator = "| ", $css_current = "", $back_forward = false, $use_images = false) {
		...
		if ($curr_pages <= $all_pages && $curr_pages >= 0) {
			...
			if($all_pages >= 1) {
				...
				$navi_string ="";
				$navi_string = ($curr_pages > 0) ?  "<a href=#  onclick=\"javascript:call('index.php?action=".$this->action."&where=".urlencode($this->where)."&".$var."=".$backward.$this->rebuild_qs($var)."',document.getElementById('".$this->div_name."'))\" >". $lbl_backward."</a>&nbsp;" :  $lbl_backward."&nbsp";
				if (!$back_forward) {
					for($a = $start + 1; $a <= $end; $a++){
						$theNext = $a - 1; // because a array start with 0
						if ($theNext != $curr_pages) {
							$navi_string .= "<a href=#  onclick=\"javascript:call('index.php?action=".$this->action."&where=".urlencode($this->where)."&".$var."=".$theNext.$this->rebuild_qs($var)."',document.getElementById('".$this->div_name."'))\">";
							$navi_string .= $a."</a>";
				...
				$navi_string .= ($curr_pages < $all_pages) ? "&nbsp;<a href=# onclick=\"javascript:call('index.php?action=".$this->action."&where=".urlencode($this->where)."&".$var."=".$forward.$this->rebuild_qs($var)."',document.getElementById('".$this->div_name."'))\">".$lbl_forward."</a>" : "&nbsp;".$lbl_forward;
			}
		}
		return $navi_string;
	}
