...
class upload {
    ...
    function init() {
        ...
        $this->file_safe_name           = true;     // format safely the filename default false
        $this->file_overwrite           = true;    // allows overwritting if the file already exists
        $this->file_auto_rename         = false;     // auto-rename if the file already exists default true
        ...
        if (!is_array($file)) {
            ...
            if ($this->uploaded) {
                ...
                switch($this->file_src_error) {
                    ...
                    case 1:
                        $this->uploaded = false;
                        $this->error = _("File upload error (Please check the file size. Max. size allowed is 2 MB)");
                        break;
                    case 2:
                        $this->uploaded = false;
    ...
    function process($server_path) {
        ...
        if ($this->uploaded) {
            ...
            else if($this->file_src_size == 0) 
            {
            	$this->processed = false;
                $this->error = _("Not a valid file");
            }
            else {
                $this->log .= '- ' . _("file size OK") . '<br />';
            }

            if($this->file_src_name_ext!="csv" && $this->file_src_name_ext!="html" && $this->file_src_name_ext!="htm"){
