...
class graph {
  //var $_SESSION= array(); // that creates a blank array
  //var $barValue = array();
  var $valuesAll = array();
  var $image;
  ...
  var $parameter         =   array(        // input parameters
    ...
    'path_to_fonts'      => 'font/',       // path to fonts folder. don't forget *trailing* slash!!
    ...
    'title_size'         =>  8,            // title text point size
    ...
    'label_colour'       => '',      		// label text colour
    ...
    'inner_border'       => 'lightblue',       // colour of border around actual graph, or 'none'.
    ...
    'y_max_left'         =>  100,            // this will be reset to maximum value if there is a value higher than this.
    ...
    'shadow_offset'      =>  1,            // draw shadow at this offset, unless overidden by data parameter.
    ...
    'axis_colour'        => '',      		// colour of axis text.
    ...
    'x_offset'           =>  0.2,          // x axis tick offset from y axis as fraction of tick spacing.
    'y_ticks_colour'     => '',       	   // colour to draw y ticks, or 'none'.
    'x_ticks_colour'     => '',      	   // colour to draw x ticks, or 'none'.
    'y_grid'             => '',       	   // grid lines. set to 'line' or 'dash'...
    'x_grid'             => '',        	   //   or if set to 'none' print nothing.
...
//Draw Area Tag Values 13-Feb-2010
function drawAreaTag() {
  $this->init();
  $this->draw_text();
  $this->draw_data();
}
...
function draw_set($order, $set, $offset) {
  ...
  foreach ($this->x_data as $index => $x) {
	...
	$values= $this->y_data[$set][$index];	// Get Values
	//if(!is_numeric($values)) return;		// if no value in DB then line will not drow 
	if( $values == -1 ){
		return;
	}
    ...
    if (($point!='none') && (string)$thisY != 'none'){ $this->plot($thisX, $thisY, $point, $pointSize, $colour, $offset, $values); $cnt++;}
...
function init_colours() {
  ...
  $this->colour['lightblue']    = ImageColorAllocate ($this->image, 72, 107, 143);
  // shades of gray
...
function plot($x, $y, $type, $size, $colour, $offset, $values) {
  ...
  switch ($type) {
    case 'square':
      ImageFilledRectangle($this->image, $u-$half, $v-$half, $u+$half, $v+$half, $this->colour[$colour]);
		   // Show Values according to the coordinates 
/*           $this->barValue = array(
									'x1'    =>  $u-$half, 
									'y1'    =>  $v-$half,
									'x2'    =>  $u+$half,
             ...
             $_SESSION['coordinates'][]=implode("|",$this->barValue);*/
      break;
    case 'square-open':
...
function getxy(){
	...
	$size = $this->parameter['point_size'];    
	$offset = $this->parameter['shadow_offset'];
	foreach ($this->y_order as $order => $set) {
   	// $this->draw_set($order, $set, 0);
		foreach ($this->x_data as $index => $x) {
			...
			// X1 X2 Axis-Corrdinates
			$thisX = $this->calculated['x_plot'][$index];
			
			// Y1 Y2 Axis-Corrdinates
			$thisY = $this->calculated['y_plot'][$set][$index];
			
			$u = $thisX + $offset;
			$v = $this->calculated['inner_border']['bottom'] - $thisY + $offset;
			$half = $size / 2;			
			// Get Score 
			$scores = $this->y_data[$set][$index];
			// Get Socre submitted Date
			$scoresDate = $this->y_scoreDate[$set][$index];
			// Get Form Type
			$formType = $this->y_formType[$set][$index];
			//echo 'X1->'.$u-$half.'  Y1->'.$v-$half.'  X2->'.$u+$half.'  Y2->'.$v+$half;
			//echo "<br>";
			$this->valuesAll[] = array($u-$half, $v-$half, $u+$half, $v+$half, $scores, $scoresDate, $formType);
			}
		}
    /*echo "<pre>";
    print_r($this->calculated['y_plot']);
    print_r($this->calculated['x_plot']);
    echo " I am here";*/
}
