<?php
//============================================================================
// OutcomeGraph Class. PHP Class to draw bar graphs.
// Version: 1.0
...
class OutcomeGraph
{
    ...
    Static $barValue = 'hello';
    
    //------------------------------------------------------------------------

   /** 
    * constructor Create an OutcomeGraph object.
    *
    ...
    * @returns null
    */
    function OutcomeGraph($width = 400, $height = 300)
    {
         register_shutdown_function(array(&$this, '_OutcomeGraph'));
    ...
    */
    function initArea()
    {
        ...
        Imageline  ( $this->img  , $this->posXStart  , $this->posYStart  , $this->posXStart , $this->posYEnd  , $this->colorLines  );
        Imageline  ( $this->img  , $this->posXStart , $this->posYEnd  , $this->posXEnd, $this->posYEnd, $this->colorLines  );
    }
...
/*        foreach($this->data as $key => $value)
        {
            if($this->textXOrientation == 'horizontal'){
                ...
                ImageString($this->img, 3, 
                $startPos+((($this->posXEnd-$this->posXStart)/$this->countData)/2)-5, 
                $this->posYEnd+5, 
                $key, $this->colorText);
    ...
    */
    function drawYAxe()
    {
        ...
        for($i=0; $i<=$this->yTicks; $i++)
        {
            ...
            //ImageLine($this->img, $this->posXStart-5, $top, $this->posXStart+0, $top, $this->colorLines);
        ...
        for($i=0; $i<=$this->yTicks; $i++) 
        {
            ImageString($this->img, 3, $this->posXStart-12-strlen($xAxeValue)*4, $top-6, $xAxeValue, $this->colorText);
    ...
    */
    function drawBars()
    {
        ...
        foreach($this->data as $key => $value) 
        {
            $barWidth = (0.55*($this->posXEnd-$this->posXStart)/$this->countData)/2;
            $barHeight = (($this->posYEnd-$this->posYStart)*$value)/$this->maxData;
            
            $x1 = $startPos + $step- $barWidth;
            $y1 = $this->posYEnd - $barHeight;
            $x2 = $startPos + $step + $barWidth;
            $y2 = $this->posYEnd;
            
            ImageFilledRectangle($this->img, $x1 , $y1 , $x2 , $y2 , $this->colorBars);
            $cnt++;
            $this->barValue[$cnt] = array(
                                                'x1'    =>  round(($startPos + $step- $barWidth)), 
                                                'y1'    =>  round(($this->posYEnd - $barHeight)),
                                                'x2'    =>  round(($startPos + $step + $barWidth)),
                                                'y2'    =>  round(($this->posYEnd)),
                                                'value' =>  $value
                                           );
            ...
            else 
            {
                ...
                /** Remove numeric figures coming on the top of bar.
    ...
    */
    function drawTitles()
    {
        ...
        //ImageStringUp($this->img, 5, $this->textPadding, $this->graphHeight/2+strlen($this->graphYTitle)*3, $this->graphYTitle, $this->colorText);
        //ImageString($this->img, 5, $this->graphWidth/2-strlen($this->graphXTitle)*3, $this->posYEnd+$this->textPadding+($this->maxTextLength*6+20)-10, $this->graphXTitle, $this->colorText);
        $this->colorText = ImageColorAllocate($this->img, 41, 138, 0);
        // Main title
        //imagettftext($this->img, 10, 0, ($this->graphWidth/2-strlen($this->graphTitle)*1)-30, $this->textPadding+15, $this->colorText, "./font/verdana.ttf", $this->graphTitle);
        // Y-axis title
        imagettftext($this->img, 10, 90, $this->textPadding + 18, ($this->graphHeight/2+strlen($this->graphYTitle)*3) + 5, $this->colorText, "./font/verdana.ttf", $this->graphYTitle);
        // X-axis title
        imagettftext($this->img, 10, 0, ($this->graphWidth/2-strlen($this->graphXTitle)) - 32, $this->posYEnd+$this->textPadding+($this->maxTextLength*6+5), $this->colorText, "./font/verdana.ttf", $this->graphXTitle);
    }
