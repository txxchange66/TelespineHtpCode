...
$API_UserName = urlencode($txxchange_config["clinicpaypalprodetails"]["API_UserName"]);
$API_Password = urlencode($txxchange_config["clinicpaypalprodetails"]["API_Password"]);
$API_Signature = urlencode($txxchange_config["clinicpaypalprodetails"]["API_Signature"]);
$environment = urlencode($txxchange_config["clinicpaypalprodetails"]["environment"]);
$currencyID = urlencode($txxchange_config["clinicpaypalprodetails"]["currencyID"]);

$application_path = $txxchange_config['application_path'];
$templatePath = $application_path . "mail_content/clinic_remindermail.php";
$templatePath_plpto = $application_path . "mail_content/plpto/clinic_creditcard_expire_notfication.php";
$templatePath_plpto1 = $application_path . "mail_content/plpto/clinic_creditcard_expire_notfication1.php";
...
function clinicCreditCardUpdateReminderMail() {
    ...
    $query = " SELECT * FROM provider_subscription WHERE subs_status='1' AND paymentType = '0' AND DATEDIFF( subs_end_datetime, NOW()) >= 0 ";
    ...
    if (@mysql_num_rows($result)) {
        while ($row = @mysql_fetch_array($result)) {
            ...
            $profileID = urlencode($row['reurring_profile_id']);
            ...
            if ($httpParsedResponseAr['EXPDATE'] == $expdate) {
                $sql = "INSERT into updated_clinic_credit_card (`paypal_id`,`userid`,`clinic_id`,`first_mail_date`,`status_mail`,`expdate`,`subscription_title`) value ('{$row['reurring_profile_id']}','{$row['user_id']}','{$row['clinic_id']}',now(),'0','{$expdate}','{$row['subscription_title']}' )";
...
function clinicCreditCardUpdateReminderMailSecondThird() {
    global $from_email_address, $templatePath, $templatePath_plpto,$templatePath_plpto1, $imagePath, $txxchange_config, $templatePath_wx, $API_UserName, $API_Password, $API_Signature, $environment;
    $expdate = date('m') . date('Y');
    $query = " SELECT * FROM updated_clinic_credit_card WHERE expdate='{$expdate}' and status_mail='0'";
    $result = @mysql_query($query);
    // taking every user's date data line by line
        ...
        while ($row = @mysql_fetch_array($result)) {
            ...
            if ($days == 10 || $days == 3 ) { //|| $days == 1
                ...
                if ($days == 10) {
                    ...
                    $message = build_template($templatePath_plpto1, $data);
                }
                //$message = build_template($templatePath,$data);
                ...
                $to = getusername($user_id);
                if ($days == 10) {
                    $subject = 'Your Credit Card on File is Expiring Soon';
                } else {
                    $subject = 'Your Credit Card on File is Expiring in a Few Days';
                }
                // To send HTML mail, the Content-type header must be set
                $headers = 'MIME-Version: 1.0' . "\n";
                $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\n";
    ...
    if (is_numeric($user_id) && $user_id > 0) {
        ...
        while ($row = @mysql_fetch_array($result)) {
            ...
            if (is_numeric($clinic_id) && $clinic_id > 0 && $field == "clinic_name") {
                $clinic_name = get_clinic_name($clinic_id, "clinic_name");
                return $clinic_name;
            }
        }
    }
...
$expdate = date('m') . date('Y');
$sql = "select * from updated_clinic_credit_card where expdate='{$expdate}'";
$res = mysql_query($sql);
$num = mysql_num_rows($res);
if ($num > 0) {
    clinicCreditCardUpdateReminderMailSecondThird($templatePath, $imagePath);
} else {
    clinicCreditCardUpdateReminderMail($templatePath, $imagePath);
}
?>
